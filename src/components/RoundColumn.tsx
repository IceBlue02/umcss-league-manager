    ...
    render() {
        ...
        return (
            <div className="roundcolumn">
                <div className="roundcolumn-title">
                    Round
                    {this.props.round.number}
                </div>
                {rows}
                <GameAddBox
                    callbacks={{ addGame: this.props.callbacks.addGame }}
                    roundno={this.props.round.number}
...
class GameAddBox extends React.Component<GameAddBoxProps, GameAddBoxState> {
    ...
    buildPlayerForm() {
        const items = [];
        let i = 1;
        const p1 = this.props.players.getPlayersWithState(PlayingState.PLAYING)[0];
