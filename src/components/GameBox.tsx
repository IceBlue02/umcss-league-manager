...
class GameBox extends React.Component<GameBoxProps> {
    ...
    onScoreChange(event: React.FormEvent<HTMLInputElement>) {
        ...
        if (event.currentTarget.name === 'upper') {
            if (AUTOFILL && this.props.game.scores.includes(null)) {
                this.props.callbacks.scoreChanged(
                    this.props.game.round - 1,
                    this.props.game.gameno - 1,
                    newValue,
                    otherValue,
                );
                this.props.callbacks.cardClick(this.props.game.round - 1, this.props.game.gameno - 1);
        ...
        } else if (AUTOFILL && this.props.game.scores.includes(null)) {
            this.props.callbacks.scoreChanged(
                this.props.game.round - 1,
                this.props.game.gameno - 1,
                otherValue,
     ...
     */
    renderGridboxRight() {
        let nameclass:[string, string] = ['name', 'name'];
        let scoreclass:[string, string] = ['score hidden', 'score hidden'];
        let inputclass:[string, string] = ['scoreinput hidden', 'scoreinput hidden'];
        ...
        return (
            <div className={`gridbox${this.props.game.isPlaying ? ' playing' : ''}`} onClick={this.onClick}>
                <div className="gridbox-left">
                    <div className="gridbox-gameno">
                        {this.props.game.round}
                        <br />
                        ·
                        <br />
                        {this.props.game.gameno}
                    </div>
                </div>
